...
export async function confirmMeal(input: unknown) {
  try {
    ...
    const db = getDb();
    await db.insert(mealLogs).values({ ownerId, rawInput: parsed.rawInput, parsedItems: parsed.items, calories: parsed.calories, protein: parsed.protein, carbs: parsed.carbs, fat: parsed.fat });
